...
sobel_dlim = 100
##
##os.makedirs("GradientIterations/Sobel",exist_ok=True)
##os.makedirs("GradientIterations/Scharr",exist_ok=True)
##for d in range(1,sobel_dlim):
##    sobel_res = I_norm_cv[:,:,target_f]
##    scharr_res = I_norm_cv[:,:,target_f]
##    for dd in range(d):
##        sobel_res = np.abs(cv2.Sobel(sobel_res,cv2.CV_8U,1,0,ksize=5))+np.abs(cv2.Sobel(sobel_res,cv2.CV_8U,0,1,ksize=5))
##        scharr_res = np.abs(cv2.Scharr(scharr_res,cv2.CV_8U,1,0))+np.abs(cv2.Scharr(scharr_res,cv2.CV_8U,0,1))
##        
##    cv2.imwrite("GradientIterations/Sobel/sobel-dx-d{}-f1048.png".format(d),sobel_res)
##    cv2.imwrite("GradientIterations/Scharr/scharr-dx-d{}-f1048.png".format(d),scharr_res)
...
for d in range(1,sobel_dlim):
    ...
    cv2.imwrite("GradientIterations/ColorSobel/sobel-col-dx-d{}-f1048.png".format(d),cv2.applyColorMap(sobel_res,cv2.COLORMAP_JET))
