...
print("Getting global median and mean Canny parameters")
...
print("Getting local median and mean Canny parameters")
# trying median and mean parameters
lt_med,ut_med = CannyMedian(I[:,:,1048])
lt_mean,ut_mean = CannyMean(I[:,:,1048])

print("Plotting Canny results using local mean and median")
# plot canny results using the parameter pairs
f,ax = plt.subplots(1,3,constrained_layout=True)
f.suptitle('Impact of Local Canny Parameters Derived from Statistics')
ax[0].contourf(I[:,:,1048].astype('uint8'))
ax[0].set_title('Frame 1048 cast to 8-bit')
ax[1].contourf(cv2.Canny(I[:,:,1048].astype('uint8'),lt_med,ut_med))
ax[1].set_title('Median')
ax[2].contourf(cv2.Canny(I[:,:,1048].astype('uint8'),lt_mean,ut_mean))
ax[2].set_title('Mean')
f.savefig('canny-local-med-mean-f1048.png')
...
## applying to normalized power density matrix
print("Normalizing power density matrix")
I_norm_cv = ((I/I.max(axis=(0,1)))*255).astype('uint8')
print("Getting global median and mean Canny parameters")
# trying median and mean parameters
lt_med,ut_med = CannyMedian(I_norm_cv)
lt_mean,ut_mean = CannyMean(I_norm_cv)

print("Plotting Canny results using mean and median")
# plot canny results using the parameter pairs
f,ax = plt.subplots(1,3,constrained_layout=True)
f.suptitle('Impact of Canny Parameters Derived from Statistics, Normalized')
ax[0].contourf(I_norm_cv[:,:,1048])
ax[0].set_title('Frame 1048 normalized and cast to 8-bit')
ax[1].contourf(cv2.Canny(I_norm_cv[:,:,1048],lt_med,ut_med))
ax[1].set_title('Median')
ax[2].contourf(cv2.Canny(I_norm_cv[:,:,1048],lt_mean,ut_mean))
ax[2].set_title('Mean')
f.savefig('canny-norm-global-med-mean-f1048.png')

print("Plotting histogram of the target frame")
# plot histogram of frame
f,ax = plt.subplots(1,2,constrained_layout=True)
f.suptitle('Histogram of normalized frame 1048')
ax[0].contourf(I_norm_cv[:,:,1048])
ax[0].set_title('Frame 1048 normalized cast to 8-bit')
ax[1].hist(I_norm_cv[:,:,1048].ravel(),256,[0,256])
ax[1].set_title('Histogram')
f.savefig('norm-cast-f1048-hist.png')

print("Getting local median and mean Canny parameters")
# trying median and mean parameters
lt_med,ut_med = CannyMedian(I_norm_cv[:,:,1048])
lt_mean,ut_mean = CannyMean(I_norm_cv[:,:,1048])
...
ax[2].contourf(cv2.Canny(I_norm_cv[:,:,1048],lt_mean,ut_mean))
ax[2].set_title('Mean')
f.savefig('canny-local-norm-med-mean-f1048.png')
...
f,ax = plt.subplots(2,2,constrained_layout=True)
f.suptitle('Histogram of normalized frame 1048')
ax[0,0].contourf(I_norm_cv[:,:,1048])
ax[0,0].set_title('Frame 1048 normalized cast to 8-bit')
ax[0,1].hist(I_norm_cv[:,:,1048].ravel(),256,[0,256])
ax[0,1].set_title('Histogram')
equ = cv2.equalizeHist(I_norm_cv[:,:,1048])
ax[1,0].contourf(equ)
ax[1,0].set_title('Frame 1048 Normalized Equalized')
ax[1,1].hist(equ.ravel(),256,[0,256])
ax[1,1].set_title('Equalized Histogram')
f.savefig('norm-equ-hist-f1048.png')
...
ax[0,1].set_title('Histogram')
clahe = cv2.createCLAHE(clipLimit=4.0,tileGridSize=(8,8))
equ_clahe = clahe.apply(I_norm_cv[:,:,1048])
ax[1,0].contourf(equ_clahe)
ax[1,0].set_title('Normalized Frame 1048 Equalized')
ax[1,1].hist(equ_clahe.ravel(),256,[0,256])
ax[1,1].set_title('Equalized Histogram')
f.savefig('equ-hist-clahe-norm-f1048.png')

## as the normalized presents a better histogram for equalization to act on
## Canny has a more impactful result
# global equalization
lt_med,ut_med = CannyMedian(equ)
lt_mean,ut_mean = CannyMean(equ)
# perform canny using median based metric
edges_med = cv2.Canny(equ,lt_med,ut_med)
edges_mean = cv2.Canny(equ,lt_mean,ut_mean)
f,ax = plt.subplots(1,3)
ax[0].contourf(equ)
ax[0].set_title('Globally Equalized Norm. Frame 1048')
ax[1].contourf(edges_med)
ax[1].set_title('Canny Edges found using Median')
ax[2].contourf(edges_mean)
ax[2].set_title('Canny Edges found using Mean')
f.savefig('global-equ-canny-norm-f0148.png')

# adaptive equalization
lt_med,ut_med = CannyMedian(equ_clahe)
lt_mean,ut_mean = CannyMean(equ_clahe)
edges_med = cv2.Canny(equ_clahe,lt_med,ut_med)
edges_mean = cv2.Canny(equ_clahe,lt_mean,ut_mean)

ax[0].contourf(equ_clahe)
ax[0].set_title('Adaptive Equalized Norm. Frame 1048')
ax[1].contourf(edges_med)
ax[1].set_title('Canny Edges found using Median')
ax[2].contourf(edges_mean)
ax[2].set_title('Canny Edges found using Mean')
f.savefig('adaptive-equ-canny-norm-f0148.png')

## trying different sigmas for the parameters
sigma = np.arange(0.05,1.0,0.05)
os.makedirs('AutoCannySigmaMean',exist_ok=True)
os.makedirs('AutoCannySigmaMedian',exist_ok=True)
print("Trying different tolerance values for auto-canny parameters on target frame")
for s in sigma:
   # find the parameters for the normed frame
   lt_med,ut_med = CannyMedian(I_norm_cv[:,:,1048],sigma=s)
   lt_mean,ut_mean = CannyMean(I_norm_cv[:,:,1048],sigma=s)
   # find edges
   edges = cv2.Canny(I_norm_cv[:,:,1048],lt_med,ut_med)
   # apply colormap
   #edges = cv2.applyColorMap(edges,plt.cm.viridis)
   cv2.imwrite('AutoCannySigmaMedian/norm-canny-median-s{0}-{1}-f1048.png'.format(*'{:.2f}'.format(s).split('.')),edges)

   # find edges
   edges = cv2.Canny(I_norm_cv[:,:,1048],lt_mean,ut_mean)
   # apply colormap
   #edges = cv2.applyColorMap(edges,plt.cm.viridis)
   cv2.imwrite('AutoCannySigmaMean/norm-canny-mean-s{0}-{1}-f1048.png'.format(*'{:.2f}'.format(s).split('.')),edges)

   ## histogram equalizations
   #global
   # find the parameters for the global equalized frame
   lt_med,ut_med = CannyMedian(equ,sigma=s)
   lt_mean,ut_mean = CannyMean(equ,sigma=s)
   # find edges
   edges = cv2.Canny(equ,lt_med,ut_med)
   # apply colormap
   #edges = cv2.applyColorMap(edges,plt.cm.viridis)
   cv2.imwrite('AutoCannySigmaMedian/norm-equ-canny-median-s{0}-{1}-f1048.png'.format(*'{:.2f}'.format(s).split('.')),edges)

   # find edges
   edges = cv2.Canny(equ,lt_mean,ut_mean)
   # apply colormap
   #edges = cv2.applyColorMap(edges,plt.cm.viridis)
   cv2.imwrite('AutoCannySigmaMean/norm-equ-canny-mean-s{0}-{1}-f1048.png'.format(*'{:.2f}'.format(s).split('.')),edges)
   
   # clahe
   # find the parameters for the adaptive equalized frame
   lt_med,ut_med = CannyMedian(equ_clahe,sigma=s)
   lt_mean,ut_mean = CannyMean(equ_clahe,sigma=s)
   # find edges
   edges = cv2.Canny(equ_clahe,lt_med,ut_med)
   # apply colormap
   #edges = cv2.applyColorMap(edges,plt.cm.viridis)
   cv2.imwrite('AutoCannySigmaMedian/norm-clahe-canny-median-s{0}-{1}-f1048.png'.format(*'{:.2f}'.format(s).split('.')),edges)

   # find edges
   edges = cv2.Canny(equ_clahe,lt_mean,ut_mean)
   # apply colormap
   #edges = cv2.applyColorMap(edges,plt.cm.viridis)
   cv2.imwrite('AutoCannySigmaMean/norm-clahe-canny-mean-s{0}-{1}-f1048.png'.format(*'{:.2f}'.format(s).split('.')),edges)

## visual inspection shows that the global histogram equalization performs better
# creating directory for results
os.makedirs('GlobalEqu',exist_ok=True)
os.makedirs('GlobalEquCannyMean',exist_ok=True)
os.makedirs('GlobalEquCannyMedian',exist_ok=True)
print("Perform global histogram equalization on the entire dataset")
for ff in range(I.shape[2]):
   # perform global equalization
   equ = cv2.equalizeHist(I_norm_cv[:,:,ff])
   # save equ results
   cv2.imwrite('GlobalEqu/norm-equ-f{0}.png'.format(ff),equ)
   # find canny parameters
   lt_med,ut_med = CannyMedian(equ,sigma=0.33)
   lt_mean,ut_mean = CannyMean(equ,sigma=0.33)
   # find edges
   edges = cv2.Canny(equ,lt_med,ut_med)
   # apply colormap
   #edges = cv2.applyColorMap(edges,plt.cm.viridis)
   cv2.imwrite('GlobalEquCannyMedian/norm-equ-canny-median-f{0}.png'.format(ff),edges)
   # using mean parameters
   edges = cv2.Canny(equ,lt_mean,ut_mean)
   # apply colormap
   #edges = cv2.applyColorMap(edges,plt.cm.viridis)
   cv2.imwrite('GlobalEquCannyMean/norm-equ-canny-mean-f{0}.png'.format(ff),edges)
