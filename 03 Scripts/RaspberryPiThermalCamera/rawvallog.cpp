 ...
 * - Delay : How long to delay the program for before starting logging in seconds.
 *
 ...
 * tc%7d.hdf5
 *
 * This is tc followed by a 7 digit number preceeded by the appropriate
 * number of zeros e.g. tc0000007.hdf5. The program iterates an integer
 * until a filename that is not being used is found.
 *
 * This was changed from the datetime format as the Raspberry Pi only updates
 * it's system date and time whenever it is connected to a network. When in
 * use the system does not always have access to a network which cause files
 * to be overriden and data lost.
 *
...
// function for formatting the filename according to a target spec
// and the passed arguments
template<typename ... Args>
std::string stringFormat(const std::string& format,Args ... args)
{
	// calculate size of buffer needed for string and arguments
	// add one for null terminator
	size_t size = std::snprintf(nullptr,0,format.c_str(),args ...)+1;
	// if the size is zero or negative them something is wrong
	// error is generated
	if(size<=0){std::runtime_error("Error during formatting!");}
	// new buffer is created to hold formatted string
	std::unique_ptr<char[]> buff(new char[size]);
	// formatt string and place in buff
	std::snprintf(buff.get(),size,format.c_str(),args ...);
	// return string minus the null terminator
	return std::string(buff.get(),buff.get() + size -1);
}

bool checkIfFileExists(const std::string& fname)
{
...
int main(int argc, char *argv[]){
	...
	auto currd = startd;
	std::cout << "Starting delay of " << timedel << " secs" << std::endl;
	while(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now()-startd).count()<timedel);
	std::cout << "Finished delay" << std::endl;
    }
    std::cout << "Finishe delay" << std::endl;
    /// Generate filename
    ...
    std::cout << "Getting pi address" << std::endl;
    //getRPiAddress(local_addr);
    // convert the address to a string
    std::string addr_str = "127.0.0.1"; 
    std::cout << "Local address is " << addr_str << std::endl;
    ...
    */
    ...
    // generate filename according to unique number
    int fnum = 0; // number to test
    std::string filename; // filename
    const std::string fnameSpec("tc%07d.hdf5"); // filename format
    while(true)
    {
	// generate filename based off spec and current value of fnum
	filename = stringFormat(fnameSpec,fnum);
	// if it doesn't exist, break from loop and use that as filename
