...
with h5py.File(path,'r') as file:
    ...
    xc,yc = np.meshgrid(np.arange(crop_r[0],crop_r[1]),np.arange(0.0,w,1.0))
...
axsb.plot_surface(xc,yc,diff[:,crop_r[0]:crop_r[1]],cmap='magma')
axlp.plot_surface(xc,yc,diff[:,crop_r[0]:crop_r[1]],cmap='magma')
axcn.plot_surface(xc,yc,diff[:,crop_r[0]:crop_r[1]],cmap='magma')
...
def update(val):
    ...
    # plot edge surfaces
    sb = sobel(diff[:,crop_r[0]:crop_r[1]])
    lp = laplace(diff[:,crop_r[0]:crop_r[1]])
    cn = canny(diff[:,crop_r[0]:crop_r[1]])
    axsb.plot_surface(xc,yc,sb,cmap='magma')
    axlp.plot_surface(xc,yc,lp,cmap='magma')
    axcn.plot_surface(xc,yc,cn,cmap='magma')

    axsb.set_title('Sobel')
    axlp.set_title('Laplace')
    axcn.set_title('Canny')
    
    ## search for interesting edges according to search condition
    # sobel
    #i,j = np.where(sb>=0.015)
    i,j = np.where((sb>3.0*sb.mean()) & (sb>=0.015))

    if (i.shape[0]>0) and (j.shape[0]>0):
        sb_ct = drawct(i,j,img[:,crop_r[0]:crop_r[1]])
        tsb.set_text("xr={} yr={}px".format(i.max()-i.min(),j.max()-j.min()))
        aximgsb.imshow(cv2.rotate(sb_ct,cv2.ROTATE_90_CLOCKWISE),cmap='gray')
    else:
        tsb.set_text("0 0px")
        aximgsb.imshow(cv2.rotate(img[:,crop_r[0]:crop_r[1]],cv2.ROTATE_90_CLOCKWISE),cmap='gray')
        
    # laplacian
    il,jl = np.where((lp>3.0*lp.mean())*(lp>0.05))

    if (il.shape[0]>0) and (jl.shape[0]>0):
        lp_ct = drawct(il,jl,img[:,crop_r[0]:crop_r[1]])
        tlp.set_text("xr={} yr={}px".format(il.max()-il.min(),jl.max()-jl.min()))
        aximglp.imshow(cv2.rotate(lp_ct,cv2.ROTATE_90_CLOCKWISE),cmap='gray')
    else:
        tlp.set_text("0 0px")
    ...
    if (ic.shape[0]>0) and (jc.shape[0]>0):
        cn_ct = drawct(ic,jc,img[:,crop_r[0]:crop_r[1]])
        tcn.set_text("xr={} yr={}px".format(ic.max()-ic.min(),jc.max()-jc.min()))
        aximgcn.imshow(cv2.rotate(cn_ct,cv2.ROTATE_90_CLOCKWISE),cmap='gray')
    else:
        tcn.set_text("0 0px")
