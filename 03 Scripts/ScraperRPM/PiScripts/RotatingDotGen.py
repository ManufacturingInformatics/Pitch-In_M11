...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Set of classes for generating a rotating dot animation for testing image processing algorithms")
    parser.add_argument("--debug",dest="debug",action="store_true",help="Flag to trigger running debug function")
    parser.add_argument("--record",dest="record",action="store_true",help="Generate rotating dot and record the footage")
    parser.add_argument("--occtest",dest="occtest",action="store_true",help="Run function for testing occlusion factory class")
    # set default responses
    parser.set_defaults(debug=False,record=False,occtest=False)
    # parse all arguments
    args = parser.parse_args()
    # if debug flag is set
    # run the debug program passing the record flag from arguments as well
    if args.debug:
        dotDebug(args.record)
    # if just the record flag is set
    elif args.record:
        dotRecord()
    # if the occlusion test flag is set
    elif args.occtest:
        dot_occlusion_test()
