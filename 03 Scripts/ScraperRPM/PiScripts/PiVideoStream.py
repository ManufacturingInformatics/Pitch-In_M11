...
class PiVideoStream:
    ...
    # method to check state of error flag
    def isError(self):
        return self.__error
    ...
    def update(self):
        ...
        for f in self.stream:
            ...
            if self.frame is None:
                print("Captured empty frame! Exiting")
                self.__error = True
                self.stopped = True
...
# inherits from PiVideoStream
class PiBufferStream(PiVideoStream):
    def __init__(self,resolution=(320,240),framerate=32,qlen=100):
        super().__init__(resolution,framerate)
        # initialize queue
...
def demo(useBuffer=True):
    if useBuffer:
        vs = PiBufferStream(resolution=(1280,720),framerate=60).start()
    else:
