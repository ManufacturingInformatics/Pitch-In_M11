...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Tracks a moving black dot using techniques from OpenCV")
    parser.add_argument("--record",dest="record",action="store_true",help="Record footage")
    parser.add_argument("--useanim",dest="useanim",action="store_true",help="Use the rotating dot animation class")
    # build string list of supported trackers
    trackstring = "".join([f"{k} : {v.__name__}" + "\n" for k,v in OPENCV_OBJECT_TRACKERS.items()])
    parser.add_argument("--tracker",dest="ttype",help="Type of tracker to use. Default is all.\n\nSupported trackers are {}".format(trackstring))
    ## parse arguments
    # check if any arguments have been parsed
    if not len(sys.argv) >1:
        parser.print_help()
    else:
        args = parser.parse_args()
        if args.tracker:
            objectTrackApp(args.tracker)
        elif not args.tracker or (args.tracker=="all"):
            objectTrackAllApp()
        elif args.useanim:
            #objectTrackAllDot(False)
            v,a=objectTrackAllDotVel(args.record)
        cv2.destroyAllWindows()
