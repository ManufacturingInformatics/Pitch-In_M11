...
params={"starter_res" : 200,    # number of data points for a 0-20 K temperature range, used to set resolution for large data sets
        ...
        "T_test_max" : 1773.15, # temperature range over which new data was predicted across
    ...
    if material in mat_dict.key():
        ...
        for f in fields:
            ...
            if vals != None:
                ...
                for cv,v in enumerate(vals):
                    # construct values matrix and add to list
                    mats.append(np.full(starter_res*(temp[cv][1]-temp[cv][0])/20,v,dtype='float'))
                # concatenate the matricies together
                full_mat = np.concatenate(mats,axis=0)
                # construct temperature matrix
                temp_mat = np.linspace(temp[0][0], # min temperature value
...
def buildMaterialData(material="SS-316L"):
    ...
    if material in mat_dict.keys():
        ...
        T_test_data = np.linspace(0,params["T_test_max"],params["starter_res"]*params["T_test_max"]/20)
        ...
        if len(p_data)==1:
            ...
            p_data,p_temp_range = genDensityData(Ev_ispline,p_data,p_temp,max_temp=p_temp.max()+past_max_temp)
        # fit spline to data
