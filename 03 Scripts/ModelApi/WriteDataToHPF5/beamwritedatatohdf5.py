""" Collection of functions used for reading, writing and searching the data generated and collected through BEAM thermal data simulations and runs to a
    ...
    Modified by David Miller, 27/09/2019
...
def addQr(h5_file,Qr,createNew=False,material="SS-316L"):
    ...
    if not "Runs" in h5_file:
        ...
        if not "Data/Camera/Qr" in run_group:
            qr_dataset = run_group["Data/Camera"].create_dataset("Qr",data=Qr,compression="gzip",compression_opts=9,track_times=True)
...
def addXML(h5_file,xml_data,createNew=False,material="SS-316L"):
    ...
    if not "Runs" in h5_file:
        ...
        if not "Data/Laser/t" in run_group:
            dataset = run_group["Data/Laser"].create_dataset("t",data=xml[1],compression="gzip",compression_opts=9,track_times=True)
        else:
        ...
        for di,d in enumerate(data_order):
            ...
            if not dpath in run_group:
                dataset = run_group["Data/Laser"].create_dataset(d,data=xml[di+2],compression="gzip",compression_opts=9,track_times=True)
    ...
    if not "Runs" in h5_file:
        ...
        if type(pi_data) is np.ndarray:
            if not "pi-camera-data-1" in pi_group:
                ...
                dataset = pi_group.create_dataset("pi-camera-data-1",data=pi_data,compression="gzip",compression_opts=9,track_times=True)
        ...
        # if it's a list of datasets
            ...
            for cd,d in enumerate(pi_data):
                if not "pi-camera-data-{0}".format(cd) in pi_group:
                    dataset = pi_group.create_dataset("pi-camera-data-{0}".format(cd),data=d,compression="gzip",compression_opts=9,track_times=True)
                    dataset.attrs["description"] = "Radiative Heat values recorded by a Raspberry Pi Thermal Camera"
                    dataset.attrs["unitType"] = "Joules/m2"
                else:
        ...
        # if it's a dictionary of datasets
            ...
            for key,val in pi_data.items():
                if not key in pi_group:
                    dataset = pi_group.create_dataset(key,data=val,compression="gzip",compression_opts=9,track_times=True)
                    dataset.attrs["description"] = "Radiative Heat values recorded by a Raspberry Pi Thermal Camera"
                    dataset.attrs["unitType"] = "Joules/m2"
                else:
    ...
    if not "Runs" in h5_file:
        ...
        if 'P' in kwargs:
            ...
            if not "Predictions/P" in run_group:
                dataset = run_group["Predictions"].create_dataset("P",data=kwargs['P'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'I' in kwargs:
            ...
            if not "Predictions/I" in run_group:
                dataset = run_group["Predictions"].create_dataset("I",data=kwargs['I'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'r0' in kwargs:
            if not "Predictions/r0" in run_group:
                dataset = run_group["Predictions"].create_dataset("r0",data=kwargs['r0'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'T' in kwargs:
            if not "Predictions/T" in run_group:
                dataset = run_group["Predictions"].create_dataset("T",data=kwargs['T'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'Tp' in kwargs:
            if not "Predictions/Tp" in run_group:
                dataset = run_group["Predictions"].create_dataset("Tp",data=kwargs['Tp'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'x' in kwargs:
            if not "Data/Laser/x" in run_group:
                dataset = run_group["Data/Laser"].create_dataset("x",data=kwargs['x'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'y' in kwargs:
            if not "Data/Laser/y" in run_group:
                dataset = run_group["Data/Laser"].create_dataset("y",data=kwargs['y'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'z' in kwargs:
            if not "Data/Laser/z" in run_group:
                dataset = run_group["Data/Laser"].create_dataset("z",data=kwargs['z'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 't' in kwargs:
            if not "Data/Laser/t" in run_group:
                dataset = run_group["Data/Laser"].create_dataset("t",data=kwargs['t'],compression="gzip",compression_opts=9,track_times=True)
                dataset.attrs["description"] = "Sample time"
                dataset.attrs["unitType"] = "seconds" 
            else:
                dataset = run_group["Data/Laser/t"]
        ...
        if 'Tq' in kwargs:
            if not "Data/Laser/T" in run_group:
                ...
                dataset = run_group["Data/Laser/Tq"]
                dataset[...] = kwargs['Tq']
        ...
        if 'vel' in kwargs:
            if not "Data/Laser/vel" in run_group:
                dataset = run_group["Data/Laser"].create_dataset("vel",data=kwargs['vel'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'Qr' in kwargs:
            if not "Data/Laser/Qr" in run_group:
                dataset = run_group["Data/Camera"].create_dataset("Qr",data=kwargs['Qr'],compression="gzip",compression_opts=9,track_times=True)
        ...
        if 'pi' in kwargs:
            ...
            if type(data) is np.ndarray:
                if not "pi-camera-data-1" in pi_group:
                    # create a single dataset called pi-camera-data-1
                    dataset = pi_group.create_dataset("pi-camera-data-1",data=data,compression="gzip",compression_opts=9,track_times=True)
                    dataset.attrs["description"] = "Radiative Heat values recorded by a Raspberry Pi Thermal Camera"
                    dataset.attrs["unitType"] = "Joules/m2"
                else:
                    dataset = pi_group["pi-camera-data-1"]
