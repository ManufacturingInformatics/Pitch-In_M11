...
os.makedirs("LogOp",exist_ok=True)
...
##    #cvlog=cv2.applyColorMap(cvlog.astype('uint8'),cv2.COLORMAP_JET)
##    cv2.imwrite("LogOp/logop-diffc-f{}.png".format(ff),cvlog.astype('uint8'))
##    res = hough_circle(cvlog.astype('uint8'),radii_range)
...
##f.suptitle('Power Estimate After Filtering by Logarithm Operator')
##f.savefig('pest-logop.png')
##np.savetxt("pest-logop-global-assumption.csv",pest,delimiter=',')

# trying a different constant
I_max = I_norm_cv.max(axis=(0,1,2))
c = 255/np.log10(I_max+1)
os.makedirs("LogOpDiffC",exist_ok=True)
...
for ff in range(I.shape[2]):
    ...
    cvlog = c*np.log10(I_norm_cv[:,:,ff]+1)
    #cvlog=cv2.applyColorMap(cvlog.astype('uint8'),cv2.COLORMAP_JET)
    cv2.imwrite("LogOpDiffC/logop-diffc-f{}.png".format(ff),cvlog.astype('uint8'))
    res = hough_circle(cvlog.astype('uint8'),radii_range)
    accums,cx,cy,radii= hough_circle_peaks(res,radii_range,num_peaks=1)
    if accums.shape[0]!=0:
        pest[ff] = powerEstHoughCircle([cx[accums.argmax()],cy[accums.argmax()],radii[accums.argmax()]],I[:,:,ff],pixel_pitch)
...
ax.plot(pest)
ax.set(xlabel='Frame Index',ylabel='Power Estimate (W)')
f.suptitle('Power Estimate After Filtering by Logarithm Operator where\nScaling Constant is based off Global Maxima')
f.savefig('pest-logop-global-maxima.png')
np.savetxt("pest-logop-global-maxima.csv",pest,delimiter=',')

os.makedirs("LogLocalMax",exist_ok=True)
...
for ff in range(I.shape[2]):
    ...
    cvlog = c*np.log10(I_norm_cv[:,:,ff]+1)
    #cvlog=cv2.applyColorMap(cvlog.astype('uint8'),cv2.COLORMAP_JET)
    cv2.imwrite("LogLocalMax/logop-local-f{}.png".format(ff),cvlog.astype('uint8'))
    res = hough_circle(cvlog.astype('uint8'),radii_range)
    accums,cx,cy,radii= hough_circle_peaks(res,radii_range,num_peaks=1)
    if accums.shape[0]!=0:
        pest[ff] = powerEstHoughCircle([cx[accums.argmax()],cy[accums.argmax()],radii[accums.argmax()]],I[:,:,ff],pixel_pitch)
...
f.savefig('pest-logop-local-maxima.png')
np.savetxt("pest-logop-local-maxima.csv",pest,delimiter=',')
...
for pi,pc in enumerate(range(-tol,tol,tol_res)):
    path = "ConstantTol/{}".format(int(pc))
    os.makedirs(path,exist_ok=True)

    c = 255/np.log10(I_norm_cv[:,:,ff].max((0,1))+1)
    c += (1/pc)*c
    cvlog = c*np.log10(I_norm_cv[:,:,ff]+1)
    cv2.imwrite(os.path.join(path,"logop-local-tol{}-f{}.png".format(int(pc),ff)),cvlog.astype('uint8'))
    res = hough_circle(cvlog.astype('uint8'),radii_range)
    accums,cx,cy,radii= hough_circle_peaks(res,radii_range,num_peaks=1)
    if accums.shape[0]!=0:
...
for pi,pc in enumerate(range(-tol,tol,tol_res)):
    ...
    fs.suptitle("Power Estimate After Filtering by a Logarithm Operator where\nScaling Constant is based off Local Maxima plus {}%".format(int(pc)))
    fs.savefig("pest-logop-local-max-diff-tol-{}.png".format(int(pc)))
...
f.savefig("pest-logop-local-max-diff-tol.png")

#################### Inverse log
# interpret as 64 bit to ensure inf doesn't happen
I_norm_cv = I_norm_cv.astype('float64')
# 255 = c*10^(r+1)
c = 255*(10**(-(255+1)))
f,ax = plt.subplots()
pest = np.zeros(I.shape[2])
for ff in range(I.shape[2]):
    print("\r{}".format(ff),end='')
    # interpret as 16-bit so the values can be incremented
    cvlog = c*10**(I_norm_cv[:,:,ff]+1)
    #cvlog=cv2.applyColorMap(cvlog.astype('uint8'),cv2.COLORMAP_JET)
    cv2.imwrite("LogOp/logop-inv-diffc-f{}.png".format(ff),cvlog.astype('uint8'))
    res = hough_circle(cvlog.astype('uint8'),radii_range)
    accums,cx,cy,radii= hough_circle_peaks(res,radii_range,num_peaks=1)
    if accums.shape[0]!=0:
        pest[ff] = powerEstHoughCircle([cx[accums.argmax()],cy[accums.argmax()],radii[accums.argmax()]],I[:,:,ff],pixel_pitch)
...
f.suptitle('Power Estimate After Filtering by Logarithm Operator')
f.savefig('pest-logop.png')
np.savetxt("pest-logop-inv-global-assumption.csv",pest,delimiter=',')

# trying a different constant based off global maxima
I_max = I_norm_cv.max(axis=(0,1,2))
c = 255*(10**(-(I_max+1)))
os.makedirs("LogOpDiffC",exist_ok=True)
print("\nTrying different constant")
for ff in range(I.shape[2]):
    ...
    cvlog = c*10**(I_norm_cv[:,:,ff]+1)
    #cvlog=cv2.applyColorMap(cvlog.astype('uint8'),cv2.COLORMAP_JET)
    cv2.imwrite("LogOpDiffC/logop-inv-diffc-f{}.png".format(ff),cvlog.astype('uint8'))
    res = hough_circle(cvlog.astype('uint8'),radii_range)
    accums,cx,cy,radii= hough_circle_peaks(res,radii_range,num_peaks=1)
    if accums.shape[0]!=0:
        pest[ff] = powerEstHoughCircle([cx[accums.argmax()],cy[accums.argmax()],radii[accums.argmax()]],I[:,:,ff],pixel_pitch)
...
f.suptitle('Power Estimate After Filtering by Logarithm Operator where\nScaling Constant is based off Global Maxima')
f.savefig('pest-logop-global-maxima.png')
np.savetxt("pest-logop-inv-global-maxima.csv",pest,delimiter=',')
...
for ff in range(I.shape[2]):
    print("\r{}".format(ff),end='')
    # interpret as 16-bit so the values can be incremented
    c = 255*(10**(-(I_norm_cv[:,:,ff].max(axis=(0,1))+1)))
    cvlog = c*10**(I_norm_cv[:,:,ff]+1)
    #cvlog=cv2.applyColorMap(cvlog.astype('uint8'),cv2.COLORMAP_JET)
    cv2.imwrite("LogLocalMax/logop-inv-local-f{}.png".format(ff),cvlog.astype('uint8'))
    res = hough_circle(cvlog.astype('uint8'),radii_range)
    accums,cx,cy,radii= hough_circle_peaks(res,radii_range,num_peaks=1)
    if accums.shape[0]!=0:
        pest[ff] = powerEstHoughCircle([cx[accums.argmax()],cy[accums.argmax()],radii[accums.argmax()]],I[:,:,ff],pixel_pitch)
...
f.suptitle('Power Estimate After Filtering by Logarithm Operator where\nScaling Constant is based off Local Maxima')
f.savefig('pest-logop-local-maxima.png')
np.savetxt("pest-logop-inv-local-maxima.csv",pest,delimiter=',')
