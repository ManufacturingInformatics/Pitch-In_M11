...
def conv2d_basic(x, W, bias): #Simple conv and biase addition this function is waste of time replace in the code with the tensorflow command
    conv = tf.nn.conv2d(input=x, filters=W, strides=[1, 1, 1, 1], padding="SAME") #Padding same mean the output is same size as input?
    return tf.nn.bias_add(conv, bias)


def conv2d_strided(x, W, b):# Simple strided convolution for transpose convolution waste of time replace in code
    conv = tf.nn.conv2d(input=x, filters=W, strides=[1, 2, 2, 1], padding="SAME")
...
def avg_pool_2x2(x): #simple tensorflow average pooling (why average?) not really needed
    return tf.nn.avg_pool2d(input=x, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")
...
def batch_norm(x, n_out, phase_train, scope='bn', decay=0.9, eps=1e-5):
    ...
    with tf.compat.v1.variable_scope(scope):
        beta = tf.compat.v1.get_variable(name='beta', shape=[n_out], initializer=tf.compat.v1.constant_initializer(0.0)
                               , trainable=True)
        gamma = tf.compat.v1.get_variable(name='gamma', shape=[n_out], initializer=tf.compat.v1.random_normal_initializer(1.0, 0.02),
                                trainable=True)
...
def bottleneck_unit(x, out_chan1, out_chan2, down_stride=False, up_stride=False, name=None):
    ...
    def conv(tensor, out_chans, shape, strides, name=None):
        ...
        return tf.nn.conv2d(input=x, filters=kernel, strides=[1, strides, strides, 1], padding='SAME', name='conv')
    ...
    with tf.compat.v1.variable_scope('res%s' % name):
        ...
        with tf.compat.v1.variable_scope('branch2a'):
            if up_stride:
        ...
        with tf.compat.v1.variable_scope('branch2b'):
        ...
        with tf.compat.v1.variable_scope('branch2c'):
