...
#-----------------------------Build network encoder based on VGG16 network and load the trained VGG16 weights-----------------------------------------
       #Layer 1
        ...
        W = tf.Variable(tf.random.truncated_normal([3, 3, 1, 64], mean=0.0, stddev=0.01, dtype=tf.float32), name="W0")
        B = tf.Variable(tf.random.truncated_normal([64], mean=0.0, stddev=0.01, dtype=tf.float32), name="B0")
        self.RelevanceMap = tf.nn.bias_add(tf.nn.conv2d(input=tf.cast(ROIMap,tf.float32), filters=W, strides=[1, 1, 1, 1], padding="SAME"),B) # apply covolution add bias
...
##-----------------------Build Net Fully connvolutional layers------------------------------------------------------------------------------------
        ...
        self.relu_dropout6 = tf.nn.dropout(self.relu6,rate=1 - (keep_prob))  # Apply dropout for traning need to be added only for training
        ...
        self.relu_dropout7 = tf.nn.dropout(self.relu7, rate=1 - (keep_prob))  # Another dropout need to be used only for training
...
#-------------------------------------Build Decoder --------------------------------------------------------------------------------------------------
        ...
        W_t1 = utils.weight_variable([4, 4, deconv_shape1[3], NUM_CLASSES],name="W_t1")  # Deconvolution/transpose in size 4X4 note that the output shape is of  depth NUM_OF_CLASSES this is not necessary in will need to be fixed if you only have 2 catagories
        b_t1 = utils.bias_variable([deconv_shape1[3]], name="b_t1")
        self.conv_t1 = utils.conv2d_transpose_strided(self.conv8, W_t1, b_t1, output_shape=tf.shape(input=self.pool4))  # Use strided convolution to double layer size (depth is the depth of pool4 for the later element wise addition
        ...
        self.conv_t2 = utils.conv2d_transpose_strided(self.fuse_1, W_t2, b_t2, output_shape=tf.shape(input=self.pool3))
        self.fuse_2 = tf.add(self.conv_t2, self.pool3, name="fuse_2")

        shape = tf.shape(input=rgb)
        W_t3 = utils.weight_variable([16, 16, NUM_CLASSES, deconv_shape2[3]], name="W_t3")
     ...
     #--------------------Transform  probability vectors to label maps-----------------------------------------------------------------
        self.Pred = tf.argmax(input=self.Prob, axis=3, name="Pred")
