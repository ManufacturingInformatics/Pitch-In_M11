...
# setup parser
parser = argsparse.ArgumentParser(description="Iterative search of waifu2x-caffe results using HDF5 results")
parser.add_argument("path",metavar='P',help="Path to HDF5 file containing temperature data")
parser.add_argument("op",metavar='op',type=str,default="scale",choices=["scale","noise"],help = "Which operation to perform")
parser.add_argument("opf_lim",metavar="oplim",type=float,help="Upper limit of operation factor")
parser.add_argument("--create_refs",metavar='cf',choices=[True,False],default=True,type=bool,help="Flag to forcefully create reference images for the data types. Default False."
parser.add_argument("--tmin",metavar='tmn',type=float,help="Min. value of temperature range in appropriate units")
parser.add_argument("--tmax",metavar='tmx',type=float,help="Max. value of temperature rannge in appropriate units")
parser.add_argument("--skip",metavar='s',type=int,default=0,help="Number of frames to skip in dataset")
...
temp_min = parser.parse_args('--tmin')
temp_max = parser.parse_args('--tmax')

# check to see if operation factor (e.g. scale factor, denoise level etc) is within range
opf_lim = parser.parse_args('opf_lim')
op_name = parser.parse_args('op')
if op_name=="scale":
    if opf_lim>10.0 or opf_lim<1.0:
        raise ValueError("Scale Factor cannot be negative or greater than 10!")
elif op_name=="noise":
    if opf_lim<0.0 or opf_lim>3.0:
        raise ValueError("Denoise Factor cannot be negative or greater than 3!")

# set operation factor min based on op type
if op_name=="scale":
    opf_min=1.0
if op_name=="noise":
    opf_min=0.0
# set waifu2x-caffe-cui argument for operation factor
if op_name=="scale":
    op_call='n'
if op_name=="noise":
    op_call='s'
...
# e.g. ::2 skip every other frame
# default arg value is 0 so a res of 1 means no frames are skipped
res = parser.parse_args('--skip')+1
# get filename
...
with open("superlog-{}.txt".format(foldername),'w') as log:
    ...
    if op_name=="scale":
        scale_vals = np.arange(opf_min,opf_lim+1,1,dtype=np.float)
    elif op_name=="noise":
        scale_vals = np.arange(opf_min,opf_lim+1,1,dtype=np.int)
                ...
                for dsetp,imgd,di in zip(dset_imgd,img_depth,depth_info):
                    ...
                    for scale in scale_vals:
                        ...
                        args = ["-i {}".format(dsetp), # source of images
                                ...
                                "-m {}".format(op_name), # type of operation
                                '--model_dir "{}"'.format(os.path.join('models',model)),
                                "-{0} {1}".format(scale), # scale factor
                                "-p gpu", # use gpu
                    ...
                    for ctr,scale in enumerate(scale_vals):
                        ...
                        if os.path.isdir(scale_dir[ctr]):
                            ...
                            for fct,ff in enumerate(filenames):
                                ...
                                # rescale values
                                # grayscale [0,255], temperature [local min,local max]
                                #frame = ((temp_max-temp_min)/(di.max-di.min))*frame[:,:,0] + temp_min
                        ...
                        f.savefig(os.path.join(target_folder,'diff-max-temp-all-{0}-{1}.png'.format(imgd.name)))
                        ...
                        f.savefig(os.path.join(target_folder,'diff-min-temp-all-{0}-{1}.png'.format(imgd.name)))
                        ...
                        f.savefig(os.path.join(target_folder,'diff-var-temp-all-{0}-{1}.png'.format(imgd.name)))
                        ...
                        f.savefig(os.path.join(target_folder,'diff-range-temp-all-{0}-{1}.png'.format(imgd.name)))
                        ...
                        f.suptitle('Variance of the Temperature Variance for Different Image {0} Factors\n Model: {1}'.format(op_name.capitalize(),model))
                        f.savefig(os.path.join(target_folder,'var-diff-var-all-{0}-{1}.png'.format(op_name,imgd.name)))

                        ax.clear()
                        ax.plot(scale_vals,var_diff_max)
                        ax.set(xlabel='Image Scale Factor',ylabel='Variance of the Difference Between Max Temp. (C)')
                        f.suptitle('Variance of the Difference Between Max. Temperatures \nfor Different Image {0} Factors, Model: {1}'.format(op_name.capitalize(),model))
                        f.savefig(os.path.join(target_folder,'var-diff-max-all-{0}-{1}.png'.format(op_name,imgd.name)))
                        ...
                        f.savefig(os.path.join(target_folder,'var-diff-min-all-{0}-{1}.png'.format(op_name,imgd.name)))
                        ...
                        f.savefig(os.path.join(target_folder,'var-diff-range-all-{0}-{1}.png'.format(op_name,imgd.name)))
                        ...
                        with open(os.path.join(target_folder,"best-{0}-{1}.txt".format(op_name.capitalize(),imgd.name)),'w') as f:
