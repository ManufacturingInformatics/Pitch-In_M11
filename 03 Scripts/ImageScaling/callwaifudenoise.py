...
# limit of numerical factor for denoising operation
noise_lim = 3.0
...
with open("superlog-{}-denoise.txt".format(foldername),'w') as log:
    ...
    # generate range of denoising values to try
    # denoise factor is applied uniformally across height and width
    noise_vals = np.arange(0,noise_lim+1,1)
    ...
    if os.path.isfile(path):
        ...
        for imgd,di in zip(img_depth,depth_info):
            ...
            target_folder = os.path.join(foldername+"-denoise","DatasetImages",imgd.name)
            ...
            # add folder path to list of reference image locations
            dset_imgd.append(target_folder)
            ...
            for model in wmodels:
                ...
                for dsetp,imgd,di in zip(dset_imgd,img_depth,depth_info):
                    ...
                    for noise in noise_vals:
                        ...
                        target_folder = os.path.join(foldername+"-denoise","DenoisedResults",model,imgd.name,"denoise-{}".format(int(noise)))
                        # create folder
                        os.makedirs(target_folder,exist_ok=True)
                        # add to list
                        noise_dir.append(target_folder)
                        ...
                        args = ["-i {}".format(dsetp), # source of images
                                ...
                                "-m noise", # type of operation
                                '--model_dir "{}"'.format(os.path.join('models',model)),
                                "-n {}".format(int(noise)), # denoise factor
                                "-p gpu", # use gpu
                        ...
                        print("Starting {:.2f}x denoising".format(noise))
                        ...
                        logwrite("Finished Waifu2x {0} {1} run".format(imgd.name,int(noise)))
                    ...
                    max_var = np.zeros((noise_vals.shape[0],depth))
                    min_var = np.zeros((noise_vals.shape[0],depth))
                    diff_var = np.zeros((noise_vals.shape[0],depth))
                    diff_range = np.zeros((noise_vals.shape[0],depth))
                    ...
                    # populate metrics matricies by iterating through each denoising factor used
                    for ctr,noise in enumerate(noise_vals):
                        logwrite("Iterating through results for {} data type".format(imgd.name))
                        # check if it exists, just to be sure
                        if os.path.isdir(noise_dir[ctr]):
                            logwrite("Found directory for {} results".format(imgd.name))
                            filenames = next(os.walk(noise_dir[ctr]))[2]
                            ...
                            for fct,ff in enumerate(filenames):
                                ...
                                if '.png' in ff:
                                    frame = cv2.imread(os.path.join(noise_dir[ctr],ff),cv2.IMREAD_ANYDEPTH | cv2.IMREAD_ANYCOLOR)
                                elif '.tif' in ff:
                                    try:
                                        frame = img_as_float(sk_imread(os.path.join(noise_dir[ctr],ff),plugin='pil'))
                                    except IOError as e:
                                        print("Error while handling {}! Image likely corrupted in some way".format(ff))
                                        logwrite("Error accessing {0} while using model {1} and dtype {2}!\n{3}\r       ".format(ff,model,imgd.name,str(e)))
                            ...
                            print("{} is not a folder!".format(noise_dir[ctr]))
                            logwrite("ERROR: {} folder of denoised images not found!".format(noise_dir[ctr]))
                        ## plot data
                        print("Plotting data for {}".format(noise))
                        logwrite("Generating plots for {}".format(imgd))
                        # creating folder for results
                        target_folder = os.path.join(foldername+"-denoise","Plots",model,imgd.name)
                        os.makedirs(target_folder,exist_ok=True)
                        # global data
                        f,ax = plt.subplots()
                        ax.plot(noise_vals,max_var,'rx')
                        ax.set(xlabel='Image Denoising Factor',ylabel='Difference between Max of Denoised and Original (C)')
                        f.suptitle('Difference Between the Maximum Temperatures of the Original\n and the Denoised Images for Different Denoising Factors ({})'.format(model))
                        f.savefig(os.path.join(target_folder,'diff-max-temp-all-denoise-{}.png'.format(imgd.name)))

                        ax.clear()
                        ax.plot(noise_vals,min_var,'rx')
                        ax.set(xlabel='Image Denoising Factor',ylabel='Difference between Min of Denoised and Original (C)')
                        f.suptitle('Difference Between the Minimum Temperatures of the Original\n and the Denoised Images for Different Denoising Factors ({})'.format(model))
                        f.savefig(os.path.join(target_folder,'diff-min-temp-all-denoise-{}.png'.format(imgd.name)))

                        ax.clear()
                        ax.plot(noise_vals,max_var,'rx')
                        ax.set(xlabel='Image Denoising Factor',ylabel='Difference between Variance of Denoised and Original (C)')
                        f.suptitle('Difference Between the Temperatrure Variance of the Original\n and the Denoised Images for Different Denoising Factors ({})'.format(model))
                        f.savefig(os.path.join(target_folder,'diff-var-temp-all-denoise-{}.png'.format(imgd.name)))

                        ax.clear()
                        ax.plot(noise_vals,diff_range,'rx')
                        ax.set(xlabel='Image Denoising Factor',ylabel='Difference between Temp. Ranges of Denoised and Original (C)')
                        f.suptitle('Difference Between the Temperatrure Ranges of the Original\n and the Denoised Images for Different Denoising Factors ({})'.format(model))
                        f.savefig(os.path.join(target_folder,'diff-range-temp-all-denoise-{}.png'.format(imgd.name)))
                        ...
                        for ctr,noise in enumerate(noise_vals):
                            ...
                            f.suptitle('Difference Between the Maximum Temperatures of the Original\n and the Denoised Image,f={0},m={1}'.format(int(noise),model))
                            f.savefig(os.path.join(target_folder,'diff-max-temp-denoise-{0}-{1}.png'.format(int(noise),imgd.name)))

                            ax.clear()
                            ax.plot(min_var[ctr,:])
                            ax.set(xlabel='Frame Index',ylabel='Difference between Min Temperatures (C)')
                            f.suptitle('Difference Between the Minimum Temperatures of the Original\n and the Denoised Image,f={0},m={1}'.format(int(noise),model))
                            f.savefig(os.path.join(target_folder,'diff-min-temp-denoise-{0}-{1}.png'.format(int(noise),imgd.name)))

                            ax.clear()
                            ax.plot(diff_var[ctr,:])
                            ax.set(xlabel='Frame Index',ylabel='Difference between Temperature Variance of Denoised and Original (C)')
                            f.suptitle('Difference Between the Temperature Variances of the Original\n and the Denoised Image,f={0},m={1}'.format(int(noise),model))
                            f.savefig(os.path.join(target_folder,'diff-var-temp-denoise-{0}-{1}.png'.format(int(noise),imgd.name)))
                        ...
                        ax.clear()
                        ax.plot(noise_vals,var_diff_var)
                        ax.set(xlabel='Image Denoising Factor',ylabel='Variance of the Temperature Variance (C)')
                        f.suptitle('Variance of the Temperature Variance for Different Image Denoising Factors\n Model: {}'.format(model))
                        f.savefig(os.path.join(target_folder,'var-diff-var-all-denoise-{}.png'.format(imgd.name)))

                        ax.clear()
                        ax.plot(noise_vals,var_diff_max)
                        ax.set(xlabel='Image Denoising Factor',ylabel='Variance of the Difference Between Max Temp. (C)')
                        f.suptitle('Variance of the Difference Between Max. Temperatures \nfor Different Image Denoising Factors, Model: {}'.format(model))
                        f.savefig(os.path.join(target_folder,'var-diff-max-all-denoise-{}.png'.format(imgd.name)))

                        ax.clear()
                        ax.plot(noise_vals,var_diff_min)
                        ax.set(xlabel='Image Denoising Factor',ylabel='Variance of the Difference Between Min Temp. (C)')
                        f.suptitle('Variance of the Difference Between Min. Temperatures \nfor Different Image Denoising Factor, Model: {}'.format(model))
                        f.savefig(os.path.join(target_folder,'var-diff-min-all-denoise-{}.png'.format(imgd.name)))

                        ax.clear()
                        ax.plot(noise_vals,var_diff_range)
                        ax.set(xlabel='Image Denoising Factor',ylabel='Variance of the Difference Between Temp. Ranges (C)')
                        f.suptitle('Variance of the Difference Between Temperature Ranges \nfor Different Image Denoising Factors, Model: {}'.format(model))
                        f.savefig(os.path.join(target_folder,'var-diff-range-all-denoise-{}.png'.format(imgd.name)))

                        ## the ideal denoise factor is the one that causes minimum change in the respective values
                        best_str = ["Denoise factor that causes min change in max vals {}\n".format(noise_vals[var_diff_max.argmin()]),
                        "Denoise factor that causes min change in min vals {}\n".format(noise_vals[var_diff_min.argmin()]),
                        "Denoise factor that causes min change in variance {}\n".format(noise_vals[var_diff_var.argmin()]),
                        "Denoise factor that causes the min change in temperature ranges {}\n".format(noise_vals[var_diff_range.argmin()])]

                        with open(os.path.join(target_folder,"best-noise-{}.txt".format(imgd.name)),'w') as f:
