...
os.makedirs('PopContour',exist_ok=True)

# laser radius
r0 = 0.00035 #m
r0_p = int(r0/pixel_pitch)
# normalize matricies
I_norm = I/I.max(axis=(0,1))
# range of circle radii to search for
radii_range = np.arange(r0_p,4*r0_p,1)
# population matrix
circle_pop = np.zeros(I.shape[:2],dtype='int64')
### axes to draw contour plot on
fig,ax = plt.subplots()
##div = make_axes_locatable(ax)
##cax = div.append_axes("right",size="7%",pad="2%")
# meshgrid of image coordinate values used to calculate if a point is inside the matrix
...
for ff in range(I.shape[2]):
    print("Frame: ",ff)
    circle_pop[:,:]=0;
    # get all circles found in the image
    accums,cx,cy,radii = hough_circle_peaks(hough_circle(I_norm[:,:,ff],radii_range),radii_range)
    # for each of the circles
    for x0,y0,r in zip(cx,cy,radii):
        # find all the points that are within range of the circle and update the pop count value
        circle_pop[np.where(((xx-x0)**2 + (yy-y0)**2)<=r**2)]+=1
    # convert the pop map to a uint image by normalizing it and converting to [0-255]
    # colorise is using viridis colormap
    img=cv2.applyColorMap(
       ((circle_pop.astype('float64')/circle_pop.max())*255).astype('uint8'),
       cmapy.cmap('viridis'))
    # save colorised image to file
    # faster than generating a plot
    cv2.imwrite('PopContour/skimage-pop-contour-f{0}.png'.format(ff),img)
